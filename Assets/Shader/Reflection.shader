Shader "LearnShader/Reflection" {
	...
	//两个Pass通用的顶点函数
	//inout修饰的参数表示既是输入也是输出
	//out修饰的参数表示输出
	//这个函数其实是来自unity Image使用的shader
	//可以用于进行模糊处理
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f_ui i):SV_Target{
				...
				float2 tuv = (i.uv - float2(0.5, 0)) * 
							fixed2(lerp(_TwistFadeOutIndentity, _TwistFadeInIndentity, fadeT), 1) + 
							float2(0.5, 0);
